...
public class Problem1 {
    ...
    private static final String OUTPUT_FILE = "Problem1.txt";
    ...
    public static void main(String args[]){
        try {
            ...
            List<String> users = getListOfLoggedInUser();
            writeResultToFile(users, OUTPUT_FILE);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static void writeResultToFile(List<String> users, String outputFile) throws Exception {
    ...
    private static List<String> getListOfLoggedInUser() {
        List<String> list = new ArrayList<>();
        for( String message: serverMessage){
            String trimMessage = message.trim().replaceAll("( )+", " ");
            if(trimMessage.contains("WELCOME")){
                String userName = trimMessage.split(" ")[1];
